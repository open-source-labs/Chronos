...
const Occupied: React.FC = React.memo(() => {
  const { setServicesData, app, example } = useContext(ApplicationContext);
  const { user, applications, getApplications, mode } = useContext(DashboardContext);
  const [modal, setModal] = useState({ isOpen: false, type: '' });
  const { appIndex } = useContext(ApplicationContext);
  ...
  // Log the applications value for debugging
  console.log('applications:', applications);

  // Ensure applications is an array
  const appsArray: any[] = Array.isArray(applications) ? applications : [];
  ...
  return (
    <div className="dashboardArea">
      ...
      <div className="cardContainer">
        ...
        {appsArray.map((application, i) => {
          // Assuming application is an array with the following indexes:
          // [0]: card name, [3]: description, etc.
          const description = application[3];
          const cardName = application[0];
          const isFiltered = cardName.toLowerCase().includes(searchTerm.toLowerCase());
          if ((!example && description === 'Example') || !isFiltered) return null;
          return (
            <ApplicationsCard
              key={crypto.randomUUID()}
              application={application}
        ...
        {/* Only render the Modal if it's open and there is a valid type */}
