...
const HealthModel_1 = __importDefault(require("../models/HealthModel"));
const ContainerInfo_1 = __importDefault(require("../models/ContainerInfo"));
const KafkaModel_1 = __importDefault(require("../models/KafkaModel"));
const KubernetesModel_1 = __importDefault(require("../models/KubernetesModel"));
// 👇 Import the *object* from healthHelpers, not a direct function
const healthHelpers_1 = __importDefault(require("./healthHelpers"));
const MetricsModel_1 = __importDefault(require("../models/MetricsModel"));
const dockerHelper_1 = __importDefault(require("./dockerHelper"));
const utilities_1 = __importDefault(require("./utilities"));
const GrafanaAPIKeyModel_1 = __importDefault(require("../models/GrafanaAPIKeyModel"));
...
mongo.connect = async ({ database }) => {
    console.log('Attempting to connect to database...');
    try {
        await mongoose_1.default.connect(database.URI);
        console.log(`MongoDB database connected at ${database.URI.slice(0, 20)}...`);
    }
    catch (error) {
        console.log('Error connecting to MongoDB:', error.message);
...
mongo.services = ({ microservice, interval }) => {
    console.log(`Saving "${microservice}" to services...`);
    const newService = { microservice, interval };
    const service = new ServicesModel_1.default(newService);
    service
        .save()
        .then(() => console.log(`Added new service "${microservice}"`))
        .catch(err => console.log(`Error saving service "${microservice}": `, err.message));
...
mongo.communications = ({ microservice, slack, email }) => {
    ...
    return function (req, res, next) {
        ...
        res.on('finish', () => {
            if (res.statusCode >= 400) {
                if (slack)
                    alert_1.default.sendSlack(res.statusCode, res.statusMessage, slack);
                if (email)
                    alert_1.default.sendEmail(res.statusCode, res.statusMessage, email);
            }
            newComms['responsestatus'] = res.statusCode;
            newComms['responsemessage'] = res.statusMessage;
...
mongo.health = async ({ microservice, interval, mode }) => {
    ...
    setInterval(() => {
        ...
        healthHelpers_1.default
            .collectHealthData()
            .then(async (healthMetrics) => {
            // Insert your logic for storing metrics
            const currentMetrics = await MetricsModel_1.default.find({ mode });
            if (currentMetrics.length !== healthMetrics.length) {
                await mongo.addMetrics(healthMetrics, mode, currentMetrics);
            }
            const HealthModel = (0, HealthModel_1.default)(`${microservice}`);
            await HealthModel.insertMany(healthMetrics);
        })
            .then(() => {
            console.log('Health data recorded in MongoDB');
...
mongo.docker = ({ microservice, interval, mode }) => {
    const pollInterval = interval || 10000;
    const containerInfo = (0, ContainerInfo_1.default)(`${microservice}`);
    dockerHelper_1.default
        .getDockerContainer(microservice)
        .then(containerData => {
        setInterval(() => {
            dockerHelper_1.default
...
mongo.saveService = (config) => {
    ...
    const service = new ServicesModel_1.default({
        microservice,
        interval: config.interval,
    });
    service
...
mongo.setQueryOnInterval = async (config) => {
    ...
    const currentMetricNames = {};
    if (config.mode === 'kafka') {
        model = KafkaModel_1.default;
        metricsQuery = utilities_1.default.helpers.kafkaMetricsQuery;
    }
    else if (config.mode === 'kubernetes') {
        model = KubernetesModel_1.default;
        metricsQuery = utilities_1.default.helpers.promMetricsQuery;
    }
    else if (config.mode === 'docker') {
        model = (0, ContainerInfo_1.default)(`${config.containerName}`);
        metricsQuery = utilities_1.default.helpers.promMetricsQuery;
    }
    else {
        throw new Error('Unrecognized mode');
    ...
    setInterval(() => {
        metricsQuery(config)
            ...
            if (config.mode === 'docker') {
                ...
                await model.insertMany(documents, (err) => {
                    if (err)
                        console.error(err);
                    else
                        console.log(`${config.mode} metrics recorded in MongoDB`);
...
mongo.addMetrics = async (healthMetrics, mode, currentMetricNames) => {
    ...
    await MetricsModel_1.default.create(newMets);
    return healthMetrics.length;
};
/** Auto-provisions Grafana dashboards for the newly collected metrics */
mongo.createGrafanaDashboards = async (config, parsedArray) => {
    try {
        console.log('In mongo.createGrafanaDashboards!!!');
        console.log('Calling utilities.getGrafanaDatasource()');
        const datasource = await utilities_1.default.helpers.getGrafanaDatasource(config.grafanaAPIKey);
        for (let metric of parsedArray) {
            console.log(`🎉 creating dashboard 🎉`);
            await utilities_1.default.helpers.createGrafanaDashboard(metric, datasource, 'timeseries', config.grafanaAPIKey);
        }
    }
    catch (err) {
        console.error('error in mongo.createGrafanaDashboards: ', err);
...
mongo.storeGrafanaAPIKey = async (config) => {
    try {
        console.log('In mongo.storeGrafanaAPIKey!!!');
        await GrafanaAPIKeyModel_1.default.create({ token: config.grafanaAPIKey });
        console.log('Grafana API Key stored in MongoDB');
    }
    catch (err) {
        console.error('error in mongo.storeGrafanaAPIKey: ', err);
