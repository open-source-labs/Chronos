...
// chronos.ts
//
const hpropagate_1 = __importDefault(require("hpropagate"));
const postgres_1 = __importDefault(require("./controllers/postgres"));
const mongo_1 = __importDefault(require("./controllers/mongo"));
const MongoClientWrapper_1 = __importDefault(require("./wrappers/MongoClientWrapper"));
const MongoServerWrapper_1 = __importDefault(require("./wrappers/MongoServerWrapper"));
const PostgresClientWrapper_1 = __importDefault(require("./wrappers/PostgresClientWrapper"));
const PostgresServerWrapper_1 = __importDefault(require("./wrappers/PostgresServerWrapper"));
const utilities_1 = __importDefault(require("./controllers/utilities"));
class Chronos {
    constructor(config) {
        if (!config) {
            throw new Error('Chronos config is undefined');
        ...
        config = utilities_1.default.helpers.validateInput(config);
        config = utilities_1.default.helpers.addNotifications(config);
        this.config = config;
    }
    /**
     * Inserts a unique x-correlating-id into the headers of each request/response.
     */
    propagate() {
        (0, hpropagate_1.default)({ propagateInResponses: true });
    }
    /**
     * Main controller for tracking the microservice.
     */
    track() {
        const { database, dockerized } = this.config;
        if (database.type === 'MongoDB') {
            mongo_1.default.connect(this.config);
            mongo_1.default.services(this.config);
            dockerized ? mongo_1.default.docker(this.config) : mongo_1.default.health(this.config);
            if (database.connection === 'REST') {
                return mongo_1.default.communications(this.config);
            }
        }
        else if (database.type === 'PostgreSQL') {
            postgres_1.default.connect(this.config);
            postgres_1.default.services(this.config);
            dockerized ? postgres_1.default.docker(this.config) : postgres_1.default.health(this.config);
            if (database.connection === 'REST') {
                return postgres_1.default.communications(this.config);
            }
        }
        else {
            throw new Error('The only allowed database types are MongoDB and PostgreSQL');
    ...
    async kafka() {
        ...
        await utilities_1.default.helpers.testMetricsQuery(this.config);
        if (this.config.database.type === 'MongoDB') {
            mongo_1.default.connect(this.config);
            mongo_1.default.serverQuery(this.config);
        }
        else if (this.config.database.type === 'PostgreSQL') {
            postgres_1.default.connect(this.config);
            postgres_1.default.serverQuery(this.config);
        }
        else {
            throw new Error('The only allowed database types are MongoDB and PostgreSQL');
        }
    ...
    async kubernetes() {
        await utilities_1.default.helpers.testMetricsQuery(this.config);
        if (this.config.database.type === 'MongoDB') {
            await mongo_1.default.connect(this.config);
            await mongo_1.default.storeGrafanaAPIKey(this.config);
            mongo_1.default.serverQuery(this.config);
        }
        else if (this.config.database.type === 'PostgreSQL') {
            postgres_1.default.connect(this.config);
    ...
    ServerWrapper(server, proto, methods) {
        const { database } = this.config;
        if (database.type === 'MongoDB') {
            return new MongoServerWrapper_1.default(server, proto, methods, this.config);
        }
        if (database.type === 'PostgreSQL') {
            return new PostgresServerWrapper_1.default(server, proto, methods, this.config);
        }
        return null;
    }
    /**
    ...
    ClientWrapper(client, service) {
        const { database } = this.config;
        if (database.type === 'MongoDB') {
            return new MongoClientWrapper_1.default(client, service, this.config);
        }
        if (database.type === 'PostgreSQL') {
            return new PostgresClientWrapper_1.default(client, service, this.config);
        }
        return null;
    }
    /**
