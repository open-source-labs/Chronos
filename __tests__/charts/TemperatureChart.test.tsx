...
import TemperatureChart from '../../app/charts/TemperatureChart';
...
describe('Temperature Chart', () => {
  ...
  beforeEach(() => {
    render(
      <HealthContext.Provider value={props}>
        <TemperatureChart sizing="solo" colourGenerator={jest.fn()} />
      </HealthContext.Provider>
    );
    graph = screen.getByTestId('Temperature Chart').firstChild;
  });
  ...
  it('Should have correct data points based off mock data', () => {
    expect(graph.calcdata[0][0].y).toBe(mockData[0].cputemp[0]);
    expect(graph.calcdata[0][0].i).toBe(0);
    expect(graph.calcdata[0][1].y).toBe(mockData[0].cputemp[1]);
